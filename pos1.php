...
<body>
	...
	<?php
    session_start();
	include($_SERVER['DOCUMENT_ROOT'] . '/Pharmacy-Management-System/Pharmacy-Management-System/PHARMACY/db_connection.php');
	// Ensure this file is included for database connection
    ...
    if (isset($_SESSION['user'])) {
        ...
        if ($result1->num_rows > 0) {
            ...
            $eid = $row1[0]; // Fetch the employee/admin ID
        } else {
            echo "User not found in the database.";
        }
        
        // Check if sid is set in the GET request
        if (isset($_GET['sid'])) {
            $sid = $_GET['sid'];
        }
        
        // Check if cid is set in the POST request
        if (isset($_POST['cid'])) {
            $cid = $_POST['cid'];
        }
        
        // Check if the 'custadd' form was submitted
        if (isset($_POST['custadd'])) {
            ...
            if ($stmt2->execute()) {
                ...
                echo "<p style='font-size:8; color:red;'>Invalid! Enter a valid Customer ID to record Sales.</p>";
            }
        }
